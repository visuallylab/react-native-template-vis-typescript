...
export type TStorageValue<Value> = {
...
export type TUsePersistStorageOptions<Value = any> = {
  version: number;
  persist?: boolean;
  migrate?: ((params: TMigrationFuncParams) => TStorageValue<Value>) | null;
...
const defaultOptions: TUsePersistStorageOptions = {
  ...
  // refer to https://github.com/mCodex/react-native-sensitive-info
  sensitive: false,
};

/**
 * usePersistStorage will return state that'll be consistent with your storage.
 * support storage migration and storing sensitive info
...
const usePersistStorage = <Value>(
  ...
  useEffect(() => {
    if (persist) {
      const setStateToStorage = async (forceValue?: TStorageValue<Value>) => {
